...
- [basics.py](basics.py): compute the interval vector of one pc set.
- [interval_vectors.py](interval_vectors.py): enumerate pc sets of size `m` (transposition-normalized with 0 fixed), print all interval vectors, or print only vectors shared by 2+ pc sets.
...
- In [interval_vectors.py](interval_vectors.py), pc sets are normalized by fixing the first pitch class as `0`.
